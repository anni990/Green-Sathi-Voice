...
ApiService.prototype.showConfirmationPopup = function(name, phone, isError) {
    ...
    // NumPad key mapping (when NumLock is OFF)
    var numPadKeyMap = {
        96: '0',  // Numpad 0 (NumLock ON)
        97: '1',  // Numpad 1 (NumLock ON)
        98: '2',  // Numpad 2 (NumLock ON)
        99: '3',  // Numpad 3 (NumLock ON)
        100: '4', // Numpad 4 (NumLock ON)
        101: '5', // Numpad 5 (NumLock ON)
        102: '6', // Numpad 6 (NumLock ON)
        103: '7', // Numpad 7 (NumLock ON)
        104: '8', // Numpad 8 (NumLock ON)
        105: '9', // Numpad 9 (NumLock ON)
        // NumLock OFF mappings
        45: '0',  // Insert -> 0
        35: '1',  // End -> 1
        40: '2',  // Down Arrow -> 2
        34: '3',  // Page Down -> 3
        37: '4',  // Left Arrow -> 4
        12: '5',  // Clear (Numpad 5 with NumLock OFF) -> 5
        39: '6',  // Right Arrow -> 6
        36: '7',  // Home -> 7
        38: '8',  // Up Arrow -> 8
        33: '9'   // Page Up -> 9
    ...
    // Handle number input with NumPad mapping (works regardless of NumLock state)
    phoneInput.addEventListener('keydown', function(e) {
        // Allow: backspace, delete, tab, escape, enter
        var allowedKeys = ['Backspace', 'Delete', 'Tab', 'Escape', 'Enter'];
        ...
        // Check if it's a numpad key (with or without NumLock)
        var keyCode = e.keyCode || e.which;
        if (numPadKeyMap.hasOwnProperty(keyCode)) {
            e.preventDefault();
            
            // Get the mapped number
            var numberToInsert = numPadKeyMap[keyCode];
            
            // Check if we've reached max length
            if (phoneInput.value.length >= 10) {
                console.log('⚠️ Max length reached (10 digits)');
                return;
            }
            ...
            var newValue = currentValue.substring(0, selectionStart) + numberToInsert + currentValue.substring(selectionEnd);
            
            // Apply new value and update cursor position
            phoneInput.value = newValue.substring(0, 10); // Limit to 10 digits
            var newCursorPos = selectionStart + 1;
            phoneInput.setSelectionRange(newCursorPos, newCursorPos);
            
            console.log('✅ NumPad key inserted:', numberToInsert, '(keyCode:', keyCode + ')');
            return;
        }
        
        // Allow regular number keys (0-9) from main keyboard
        if (/^[0-9]$/.test(e.key)) {
            // Let default behavior handle it, but check length
            if (phoneInput.value.length >= 10 && phoneInput.selectionStart === phoneInput.selectionEnd) {
                e.preventDefault();
                console.log('⚠️ Max length reached (10 digits)');
            }
            return;
        ...
        e.preventDefault();
        console.log('⚠️ Blocked key:', e.key, '(keyCode:', keyCode + ')');
    });
