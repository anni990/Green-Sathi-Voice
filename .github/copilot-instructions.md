...
This is a Flask-based multilingual voice assistant focused on Indian languages (Hindi, Bengali, Tamil, Telugu, Gujarati, Marathi). It uses MongoDB for data storage, Google's Gemini AI for conversation and extraction, and integrates speech-to-text/text-to-speech capabilities.

**Core Flow:** User speaks → STT → Gemini extracts name/phone → Language detection → TTS response → Conversation loop

### Key Components
- **Flask App Structure**: Blueprint-based routing (`voice_routes.py`, `user_routes.py`, `admin_routes.py`)
- **Service Layer**: `gemini_service.py` (AI interactions), `speech_service.py` (audio processing)
...
3. **Environment vars**: Copy `.env.example` and configure `GEMINI_API_KEY`, `MONGODB_URL`
